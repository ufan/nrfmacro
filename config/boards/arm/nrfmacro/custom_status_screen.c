...
lv_obj_t *zmk_display_status_screen() {
    ...
    lv_obj_align(zmk_widget_battery_status_obj(&battery_status_widget), NULL, LV_ALIGN_IN_BOTTOM_MID, 0, -2);
#else
...
#if IS_ENABLED(CONFIG_NRFMACRO_ROTATE_SCREEN)
    lv_obj_align(zmk_widget_output_status_obj(&output_status_widget), NULL, LV_ALIGN_IN_BOTTOM_MID, 0, -41);
#else
...
#if IS_ENABLED(CONFIG_NRFMACRO_ROTATE_SCREEN)
    lv_obj_align(zmk_widget_layer_status_obj(&layer_status_widget), NULL, LV_ALIGN_IN_TOP_MID, 0, 5);
#else
...
#if IS_ENABLED(CONFIG_NRFMACRO_ROTATE_SCREEN)
    lv_obj_align(zenlogo_icon, NULL, LV_ALIGN_IN_TOP_MID, 2, 5);
#else
